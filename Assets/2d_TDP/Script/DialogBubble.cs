...
public class DialogBubble : MonoBehaviour
{
    ...
    public event ClosedDelegate OnClose;
		...
		foreach (PixelBubble vBubble in vcharacter.vBubble)
		{
			...
			if (vcharacter.vCurrentBubble == null)
			{
				...
				if (vBubble.vMessageForm == BubbleType.Rectangle)
				{
					...
					vBubbleObject.transform.position = vcharacter.transform.position + new Vector3(1.35f, 1.9f, 0f) + BubbleOffset; //move a little bit the teleport particle effect
				}
				else 
				{
					//create bubble
					vBubbleObject = Instantiate(Resources.Load<GameObject> ("Customs/BubbleRound"));
					vBubbleObject.transform.position = vcharacter.transform.position + new Vector3(0.15f, 1.75f, 0f) + BubbleOffset; //move a little bit the teleport particle effect
				}
